...
export const poolDailyTvls = memoize(
  async (address: any) => {
    const qStr = (offset: number) => `dailyTvls(orderBy: timestamp_DESC, offset: ${offset}, limit: ${LIMIT}, where: {swap: {id_eq: "${address}"}})`
    const query = `{
...
export const poolDailyVolumes = memoize(
  async (address: any) => {
    const qStr = (offset: number) => `dailyVolumes(orderBy: timestamp_DESC, offset: ${offset}, limit: ${LIMIT}, where: {swap: {id_eq: "${address}"}})`
    const query = `{
